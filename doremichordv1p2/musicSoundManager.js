...
class MusicSoundManager {
    constructor() {
        ...
        // Harmonic mode: 'diatonic', 'jazz7', or 'jazz79'
        this.harmonicMode = 'diatonic'; // Default to diatonic
    ...
    // Get Diatonic extensions (no extensions - just triads)
    getDiatonicExtensions(noteIndex) {
        return []; // No extensions, just basic triads
    }

    // Get Jazz7 extensions for a note index (just 7th extensions)
    getJazz7Extensions(noteIndex) {
    ...
    // Get Jazz79 extensions for a note index (7th and 9th extensions)
    ...
    // Get extensions based on current harmonic mode
    getExtensionsForCurrentMode(noteIndex) {
        switch (this.harmonicMode) {
            case 'diatonic':
                return this.getDiatonicExtensions(noteIndex);
            case 'jazz7':
                return this.getJazz7Extensions(noteIndex);
            case 'jazz79':
            default:
                return this.getJazz79Extensions(noteIndex);
        }
    }

    // Set harmonic mode
    setHarmonicMode(mode) {
        if (['diatonic', 'jazz7', 'jazz79'].includes(mode)) {
            this.harmonicMode = mode;
        }
    }
    ...
    startChord(noteIndex, noteKey) {
        ...
        // Get extensions based on current harmonic mode
        const extensions = this.getExtensionsForCurrentMode(noteIndex);
